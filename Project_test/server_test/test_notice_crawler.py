...
class TestLMSCrawler(unittest.TestCase):
    ...
    def test_crawl_all_notices_success_integration(self, mock_get_notices, mock_get_courses):
        ...
        result = crawl_all_notices(self.mock_session, student_id="202600001")
        
        self.assertEqual(len(result), 2)
        self.assertEqual(result[0]['notice_id'], 'n2')
        self.assertEqual(result[1]['notice_id'], 'n1')

    @patch('notice_crawler.get_enrolled_courses')
    @patch('notice_crawler.get_notices_for_course')
    def test_crawl_all_notices_partial_failure(self, mock_get_notices, mock_get_courses):
        ...
        with self.assertRaises(SessionExpiredError):
            crawl_all_notices(self.mock_session, "202600001")

    @patch('notice_crawler.get_enrolled_courses')
    @patch('notice_crawler.get_notices_for_course')
    def test_crawl_all_notices_general_exception_worker(self, mock_get_notices, mock_get_courses):
        ...
        result = crawl_all_notices(self.mock_session, "202600001")
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]['notice_id'], 'n2')

    @patch('notice_crawler.get_enrolled_courses')
    def test_crawl_all_notices_empty_course(self, mock_get_courses):
    ...
    @patch('notice_crawler.get_enrolled_courses')
    @patch('notice_crawler.get_notices_for_course')
    def test_crawl_all_notices_default_board_ids(self, mock_get_notices, mock_get_courses):
        ...
        self.assertIn('common_board_ids', kwargs)
        self.assertEqual(kwargs['common_board_ids'], {'17'})


    # ─── get_notices_for_course 테스트 영역 ───────────────────────────────────

    def test_get_notices_for_course_session_expired(self):
        ...
        with self.assertRaises(SessionExpiredError):
            get_notices_for_course(self.mock_session, "course123", "과목A")

    @patch('notice_crawler.BeautifulSoup')
    def test_get_notices_for_course_no_board_id(self, mock_bs):
        ...
        result = get_notices_for_course(self.mock_session, "course123", "과목A")
        self.assertEqual(result, [])

    @patch('notice_crawler.BeautifulSoup')
    def test_get_notices_for_course_board_session_expired(self, mock_bs):
        ...
        self.mock_session.get.side_effect = [mock_resp_home, mock_resp_board]
    ...
    @patch('notice_crawler.BeautifulSoup')
    def test_get_notices_for_course_general_exception(self, mock_bs):
        """공지 목록을 파싱하는 도중 예상치 못한 에러가 잡혔을 때 로깅 후 빈 리스트를 반환하는가"""
        self.mock_session.get.side_effect = Exception("Fatal Network Error")
        result = get_notices_for_course(self.mock_session, "course123", "과목A")
        self.assertEqual(result, [])

    @patch('notice_crawler.BeautifulSoup')
    def test_get_notices_for_course_success_parsing(self, mock_bs):
        ...
        notices = get_notices_for_course(self.mock_session, "course_123", "소프트웨어공학")

        self.assertEqual(len(notices), 2)
        self.assertEqual(notices[0]['notice_id'], "9999")
        self.assertEqual(notices[1]['notice_id'], "8888")


    # ─── get_notice_detail 테스트 영역 ────────────────────────────────────────

    def test_get_notice_detail_session_expired(self):
        """공지 상세 내용 조회 시 세션 만료 응답 코드를 잡아서 처리하는가"""
        mock_resp = MagicMock(status_code=401)
        self.mock_session.get.return_value = mock_resp

        with self.assertRaises(SessionExpiredError):
            get_notice_detail(self.mock_session, "board123", "notice123")

    @patch('notice_crawler.BeautifulSoup')
    def test_get_notice_detail_not_found(self, mock_bs):
        ...
        self.assertIn("존재하지 않는 공지사항입니다", str(context.exception))

    @patch('notice_crawler.BeautifulSoup')
    def test_get_notice_detail_success_parsing(self, mock_bs):
        ...
        result = get_notice_detail(self.mock_session, "board123", "notice123")
        
        self.assertEqual(result['title'], "팀 NULL 프로젝트 공지사항")
        self.assertEqual(result['writer'], "관리자")
        self.assertEqual(result['views'], "45")
        self.assertEqual(result['attachments'][0]['url'], "https://lms.chungbuk.ac.kr/mod/ubboard/download.php?id=1")
        
        self.assertEqual(img_store['src'], "https://lms.chungbuk.ac.kr/images/test.png")

    @patch('notice_crawler.BeautifulSoup')
    def test_get_notice_detail_no_content_or_files(self, mock_bs):
        ...
        self.assertEqual(result['title'], "텍스트 제목")
    ...
    def test_crawl_all_messages_session_expired(self):
        """쪽지함 리스트 요청 시 세션 만료 응답 상태 코드가 반환되면 예외를 정상 처리하는가"""
        mock_resp = MagicMock(status_code=302)
        self.mock_session.get.return_value = mock_resp

        with self.assertRaises(SessionExpiredError):
            crawl_all_messages(self.mock_session)

    @patch('notice_crawler.BeautifulSoup')
    def test_crawl_all_messages_parsing(self, mock_bs):
        ...
        messages = crawl_all_messages(self.mock_session)
        
        self.assertEqual(len(messages), 1)
        self.assertEqual(messages[0]['sender'], "담당교수A")
        self.assertEqual(messages[0]['content'], "테스트 쪽지 내용입니다.")
        self.assertEqual(messages[0]['url'], "https://lms.chungbuk.ac.kr/local/ubsend/message/detail.php")

    @patch('notice_crawler.BeautifulSoup')
    def test_crawl_all_messages_empty_or_exception(self, mock_bs):
        ...
        self.mock_session.get.side_effect = Exception("Crash Message Center")
        messages = crawl_all_messages(self.mock_session)
        self.assertEqual(messages, [])
